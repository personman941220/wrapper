...
window.onload = function() {
	...
	// 	data : {}
	// }));
	setTimeout("a_click()",500);
	//console.log("ghdwmc");

	//window.location.href("http://113.16.167.157:9011/SKServer/zzp_spbm/init.do?target=navTab&rel=zzp_zsfpkj_nav");
	
};
function a_click(){
	$('a').each(function(){
		var href = $(this).attr('href');
		if(localStorage.getItem('zpORpp') === "zp"){
			if(href === "/SKServer/zzp_spbm/init.do?target=navTab&rel=zzp_zsfpkj_nav"){
				$(this).click();
				//console.log(this);
				setTimeout("send_tt()",500);
				return;
			}
		}
		else if(localStorage.getItem('zpORpp') === "pp"){
			if(href === "/SKServer/zzspp_spbm/init.do?target=navTab&rel=zzspp_zsfpkj_nav"){
				$(this).click();
				//console.log(this);
				setTimeout("send_tt()",500);
				return;
...
function send_tt(){
	...
	$('#ghdwmc').val(localStorage.getItem('ghdwmc')); 
	$('#ghdwdm').val(localStorage.getItem('ghdwdm'));
	$('#ghdwdzdh').val(localStorage.getItem('ghdwdzdh'));
	$('#ghdwyhzh').val(localStorage.getItem('ghdwyhzh'));
	// console.log(localStorage.getItem('ghdwmc'));
